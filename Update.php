...
$resultat->execute() or die("Erreur lors de l'execution de la requete: ".mysql_error());
	...
	if($resultat->rowCount())
	{
		...
		$sql = "SELECT comm.id id_comm, comm.nom nom_comm, c.id id_c, c.nom nom_c, ar.id id_ar, ar.nom nom_ar  FROM commune comm,conduite c,arrondissement ar where comm.id_conduite = c.id and c.id_commune = ar.id and comm.id = $id_zone ";
		$resultat = $connect->prepare($sql);
		$resultat->execute() or die("Erreur lors de l'execution de la requete: ".mysql_error());
